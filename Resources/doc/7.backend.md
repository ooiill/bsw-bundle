...
每个功能模块中都会定义一些在你控制器中可以调用的函数，这里我们统一称之为 **流函数**，这些函数有且仅有一个对象参数 `Leon\BswBundle\Module\Bsw\Arguments $args` 。因为每个流函数的参数都不尽相同，所以设计的时候干脆就作为一个对象传入，如果你不知道对象中有哪些属性，你可以直接打印它进行查看。  
...
## 常用三大功能模块之 `Bsw\Persistence` 功能模块
